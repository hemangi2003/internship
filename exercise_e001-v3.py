...
class Product:
    ...
    def product_stock_details():
        for i in P:
            ...
            print("Product Stock at locations:", i.stock_at_locations)
            print("\n")
...
class Movement:
    ...
    def movements_by_product(p):
        for x in range(len(Movements)):
            if Movements[x].product.name == p:
                print("From ", Movements[x].from_location.name, "to ", Movements[x].to_location.name,"\n",
                      "Product:-", Movements[x].product.name, ",","quantity:", Movements[x].quantity)
